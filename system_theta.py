...
class system_theta():
    def __init__(self, theta, N=None, U=None):
        ...
        #get deltaK from theory, using perhaps different intralayer hopping. This is the macdonald convention
        self.K = 4 * np.pi / 3
        self.vf = np.sqrt(3) * 2.7 / 2
    ...
    def get_bands(self, number_of_bands, window):
        ks = generate_macpath(10000, self.deltaK, self.Qs, starting_cell=(0, 0))
    ...
    def band_diff_at_cone(self):
        h = 0.5 * self.deltaK
        v1 = band_energy(0, 0, self.Qs, self.K, self.t, self.theta, self.vf, U=self.U, N=self.N)[self.midband - 1]
        v2 = band_energy(0, -h, self.Qs, self.K, self.t, self.theta, self.vf, U=self.U, N=self.N)[self.midband - 1]
        return v2-v1
